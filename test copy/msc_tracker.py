...
def get_msc_tracking(container_no: str, headless: bool = False):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Press Enter instead of clicking explicitly, to avoid disabled button on blur
            page.press("#trackingNumber", "Enter")
            print("🔍 Search submitted via Enter")
            
            # Optional click fallback if Enter wasn't enough
            try:
        ...
        try:
            ...
            for _ in range(20):  # ~20 seconds
                if page.locator(".msc-flow-tracking__details").first.is_visible():
                    time.sleep(3) # Wait for vue render
                    found = True
                    break

                # Support checking for no results scenario if applicable
            ...
            if details_list:
                ...
                for item in list_items:
                    ...
                    if heading and value:
                        ...
                        # Remove hidden template attributes like '{xxx}' if they got captured

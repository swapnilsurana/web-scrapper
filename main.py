...
CARRIER_LOOKUP = {_normalize_key(name): name for name in CARRIERS}
...
def track(request: TrackRequest, key: str = Security(verify_api_key)):
    carrier_key = _normalize_key(request.carrier)
    ...
    carrier = CARRIER_LOOKUP.get(carrier_key)
    tracker = CARRIERS.get(carrier) if carrier else None
